...
// ⬇️ Make sure the path matches your filename exactly.
// If your file is src/components/laserProducts.js, use the lowercase import:
...
export default function LaserPrinter() {
  ...
  // 🔒 Scoped CSS (same as ProductGallery/Inkjet/Office pages)
  const styles = `
    .pm-card {
      border-radius: 14px;
  ...
  // ✅ Pass the full product through router state
  const goToDetail = (product) =>
    navigate(`/product/${product.id}`, {
      state: { product, preferCategory: product.category },
    });
    ...
    <>
      ...
      <div style={{ background: "#f6f8fb" }}>
        <Container style={{ paddingTop: 20, paddingBottom: 36 }}>
          ...
          {/* Product Grid — consistent with ProductGallery */}
          <Row className="g-4">
            {sorted.map((p) => (
              <Col key={p.id} xs={12} sm={6} lg={4} xl={3}>
                <Card className="pm-card" onClick={() => goToDetail(p)}>
                  {/* Image top (no crop) */}
                  ...
                  <Card.Body style={{ display: "flex", flexDirection: "column", flex: 1 }}>
                    ...
                    <div className="pm-actions">
                      ...
                      <Button
                        ...
                        as={Link}
                        to={`/product/${p.id}`}
                        state={{ product: p, preferCategory: p.category }}
                      >
                        <FiEye />
                      </Button>
                    </div>
